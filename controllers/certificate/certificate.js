...
//   const qrData = `${config.algorand.explorerTransactionUrl}/${data.blockchain_tx}`;
...
async function generateCertificate(data, outputPath = 'certificate.png') {
  ...
  const contentSvg = `
    <svg width="720" height="380">
      ...
      <text x="500" y="230" class="value">${config.algorand.networkName}</text>
...
async function uploadToPinata(filePath) {
  ...
  const res = await axios.post(config.storage.pinataApiUrl, data, {
    maxContentLength: 'Infinity',
    headers: {
      ...data.getHeaders(),
      Authorization: `Bearer ${config.storage.pinataJwt}`,
    },
  });

  const cid = res.data.IpfsHash;
  return `${config.storage.pinataGatewayUrl}/${cid}`;
}
