...
async function getAnchor(req, res, next) {
  ...
  if (!receipt) return res.status(404).json({ success: false, code: 'ANCHOR_NOT_FOUND', message: 'Anchor not found' });
...
async function verifyAnchor(req, res, next) {
  ...
  const receipt = await receiptStore.findByAnchorId(anchorId);
  if (!receipt) return res.status(404).json({ success: false, code: 'ANCHOR_NOT_FOUND', message: 'Anchor not found' });
  const { verifyHash, EVENT_BY_CODE } = require('../services/blockchain/eventTypes');
...
async function getBlockchainHealth(req, res, next) {
  ...
  if (!config.algorand.enabled) {
    return res.json({ success: true, data: { enabled: false, status: 'disabled' } });
  }
  ...
  return res.json({ success: true, data: {
    ...
    reachable,
    address: adapter.platformAccount.addr,
...
module.exports = { getAnchor, listAnchors, verifyAnchor, getBlockchainHealth };
